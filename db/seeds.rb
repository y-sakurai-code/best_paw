...
review_list = [
  { user: users[:u1], dogs: [dogs[:d1]], genre: genres[:g2], item: "ロイヤルナン 大型犬専用フード", star: 5.0, cat: "プレミアム", title: "食いつきが全然違います！", body: "大型犬用のこのフードに変えてから、レオの毛並みが驚くほどツヤツヤになりました。以前は食後に少しお腹を壊しやすかったのですが、これにしてからは便の状態も非常に安定しています。粒の大きさも大型犬がしっかり噛めるサイズ感で、毎日の食事が楽しみなようです。少し価格は張りますが、愛犬の健康を考えると、これからも継続して購入したいと思える納得のクオリティです。" },
...
review_list.each_slice(5) do |batch|
  batch.each do |data|
    r = Review.find_or_create_by!(title: data[:title], user: data[:user]) do |review|
      ...
      review.dog_ids = data[:dogs].map(&:id)
    end
